...
/* The pre－compiled 'CG' program should be put in current directory together with 'Tool/BactCG2.0/bin' folder where pre-compiled 'lenExt',
   ...
   The program will generate a new sub-folder 'Tool/BactCG2.0/result' in current directory, and the final core gene set file could be found
   in the directory './Tool/BactCG2.0/result/cg_result/' and is named as 'CG.tab.txt'.
*/
...
func main() {
	...
	cmd := exec.Command("mkdir", "Tool/BactCG2.0/result")
	err0 := cmd.Run()
	if err0 != nil {
		fmt.Printf("Error: %s\n", err0)
	}

	cmd = exec.Command("mkdir", "Tool/BactCG2.0/result/db")
	err0 = cmd.Run()
	if err0 != nil {
		fmt.Printf("Error: %s\n", err0)
	}

	cmd = exec.Command("mkdir", "Tool/BactCG2.0/result/blast_out")
	err0 = cmd.Run()
	if err0 != nil {
		fmt.Printf("Error: %s\n", err0)
	}

	cmd = exec.Command("mkdir", "Tool/BactCG2.0/result/prot_len")
	err0 = cmd.Run()
	if err0 != nil {
		fmt.Printf("Error: %s\n", err0)
	}

	cmd = exec.Command("mkdir", "Tool/BactCG2.0/result/out_best")
	err0 = cmd.Run()
	if err0 != nil {
		fmt.Printf("Error: %s\n", err0)
	}

	cmd = exec.Command("mkdir", "Tool/BactCG2.0/result/out_best_filt")
	err0 = cmd.Run()
	if err0 != nil {
		fmt.Printf("Error: %s\n", err0)
	}

	cmd = exec.Command("mkdir", "Tool/BactCG2.0/result/out_mutbest_filt")
	err0 = cmd.Run()
	if err0 != nil {
		fmt.Printf("Error: %s\n", err0)
	}

	cmd = exec.Command("mkdir", "Tool/BactCG2.0/result/cg_result")
	err0 = cmd.Run()
	if err0 != nil {
		fmt.Printf("Error: %s\n", err0)
	}
	...
	for _, seqFile := range seqFiles {
		if len(seqFile) > 0 {
			...
			lengthFile := "./Tool/BactCG2.0/result/prot_len/" + filePrefix + ".prot_len.txt"
			cmd = exec.Command("./Tool/BactCG2.0/bin/lenExt", seqFile)
			...
			db_name = "./Tool/BactCG2.0/result/db/" + filePrefix
	...
	refdb := "./Tool/BactCG2.0/result/db/" + ref
	...
	for _, seqFile := range seqFiles {
		if len(seqFile) > 0 {
			...
			filePrefix = strings.Replace(filePrefix, seqDir, "", -1)
			filePrefix = strings.Replace(filePrefix, "/", "", -1)
			q_blastout = "./Tool/BactCG2.0/result/blast_out/" + filePrefix + ".vs." + ref + ".blastout.m6.txt"
			db_name = "./Tool/BactCG2.0/result/db/" + filePrefix
			q_best = "./Tool/BactCG2.0/result/out_best/" + filePrefix + ".vs." + ref + ".bestHit.txt"
			s_blastout = "./Tool/BactCG2.0/result/blast_out/" + ref + ".vs." + filePrefix + ".blastout.m6.txt"
			s_best = "./Tool/BactCG2.0/result/out_best/" + ref + ".vs." + filePrefix + ".bestHit.txt"
			lengthFile1 := "./Tool/BactCG2.0/result/prot_len/" + filePrefix + ".prot_len.txt"
			lengthFile2 := "./Tool/BactCG2.0/result/prot_len/" + ref + ".prot_len.txt"
			q_best_filt = "./Tool/BactCG2.0/result/out_best_filt/" + filePrefix + ".vs." + ref + ".best.filt.txt"
			s_best_filt = "./Tool/BactCG2.0/result/out_best_filt/" + ref + ".vs." + filePrefix + ".best.filt.txt"
			out_mutbest_filt = "./Tool/BactCG2.0/result/out_mutbest_filt/" + ref + ".vs." + filePrefix + ".mutbest.filt.txt"
			...
			if !strings.EqualFold(filePrefix, ref) {
				...
				cmd1 := exec.Command("./Tool/BactCG2.0/bin/bestpicker", q_blastout)
				...
				cmd2 := exec.Command("./Tool/BactCG2.0/bin/bestpicker", s_blastout)
				...
				cmd1 = exec.Command("./Tool/BactCG2.0/bin/simcovfilter", lengthFile2, q_best, sim, cov)
				buf, err1 = cmd1.Output()
				if err1 != nil {
					log.Fatal(err1)
				} else {
				...
				cmd2 = exec.Command("./Tool/BactCG2.0/bin/simcovfilter", lengthFile1, s_best, sim, cov)
				buf, err1 = cmd2.Output()
				if err1 != nil {
					log.Fatal(err1)
				} else {
				...
				cmd := exec.Command("./Tool/BactCG2.0/bin/mutbest", s_best_filt, q_best_filt)
				buf, err1 = cmd.Output()
				if err1 != nil {
					log.Fatal(err1)
				} else {
				...
				if seqOrder == 1 {
					...
					cgFile += ".cg.txt"
					cmd = exec.Command("./Tool/BactCG2.0/bin/combSingleGeneOrtho", out_mutbest_filt0, out_mutbest_filt, ">", cgFile)
					buf, err1 = cmd.Output()
					if err1 != nil {
						log.Fatal(err1)
					} else {
					...
					cgFile = strconv.Itoa(seqOrder)
					cgFile += ".cg.txt"
					cgFile = "./Tool/BactCG2.0/result/cg_result/" + cgFile
					cmd = exec.Command("./Tool/BactCG2.0/bin/combSingleGeneOrtho", cgFile0, out_mutbest_filt)
					buf, err1 = cmd.Output()
					if err1 != nil {
						log.Fatal(err1)
					} else {
	...
	cgLines := strainNames + "\n"
	cgResult := "./Tool/BactCG2.0/result/cg_result/" + "CG.tab.txt"
...
func writeFile(newFile string, seq string) {
	...
	defer outFile.Close()
	if err != nil {
		fmt.Printf("Errors happened in creating Tool/BactCG2.0/result files!\n")
	}
