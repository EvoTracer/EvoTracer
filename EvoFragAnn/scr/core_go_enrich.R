...
#读取手动准备好的背景基因集
gene_GO <- read.csv('~/工作数据存放/202602214-ag_function_enrich/input/background_go.csv',
                    header = TRUE,
                    stringsAsFactors = FALSE)
colnames(gene_GO)
colnames(gene_GO) <- c("gene","term")
colnames(gene_GO)
...
#读取基因列表文件中的基因名称
accessory_genes <- read.delim('~/工作数据存放/202602214-ag_function_enrich/input/gene.txt', 
...
#输出
write.table(accessory_GO, "/mnt/md0/Users/sunleting/20250725-g-profiler/output/core/core_go_enrich.txt", 
...
# 
output_dir<-""
ggsave(filename = file.path(output_dir, "5core_go_barplot_allterms.pdf"), 
...
ggsave(filename=file.path(output_dir,"core_go_cnetplot.pdf"),
       plot=cnet_plot,
       width = 20, 
       height =9)
...
dot_plot_all<-dotplot(accessory_go_rich, showCategory = 20, title = "GO enrichment (all terms)") +
  theme_minimal()
ggsave(filename=file.path(output_dir,"core_go_dotplot.pdf"),
       plot=dot_plot_all,
       width = 9, 
       height = 7)
