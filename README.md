...
3. **运行分析**：
```bash
./Evocg run --ref "Ref_Name" -i "./input/seq/" -o "./output/CG_results/"
```
...
* **使用步骤**：
1. **准备输入**：系统发育树目录、各节点基因序列目录、菌株 ID 映射表 `EvoID.txt`。
2. **运行分析**：
```bash
./EvoAG -tree ./input/tree_file -gene ./input/gene -id ./codes/EvoID.txt
```
...
2. **运行自动化流水线**：
```bash
./bin/Evopga pipeline -gbk ./input/nA_AG.gbk -pg ./input/26_PG.txt -seq ./input/seq -out ./output
```
...
2. **运行命令**：
```bash
onedgr <Ancient_Node_ID> --Evoid ./EvoID.txt -f ./ag_output -o MyResults -w 8
```
