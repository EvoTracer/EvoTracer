...
EvoCG 是一个用于基因组和核心基因（Core Gene）分析的快速处理管线工具包。本项目将原先分散的 Python 脚本、Perl 脚本以及 EvoCG 2.0 的各组件模块统一重构、编译并打包成了一个独立的高性能 Go 命令行程序。由于其使用了通用的相对路径体系，它非常适合分发装箱，在不同设备间可以零修改无缝迁移。
...
### 1. 运行核心自动化流水线 (替代 1.cd-hit_and_EvoCG.sh)
...
**可用的流水线参数：**
* `-r, --ref`：EvoCG 指定的参考菌株名字
* `--cd-c`：cd-hit `-c` 阈值参数（默认 `"0.7"`）
* `--cd-s`：cd-hit `-s` 阈值参数（默认 `"0.7"`）
* `--cg1`：EvoCG 第一个参数（默认 `"0.8"`）
* `--cg2`：EvoCG 第二个参数（默认 `"0.9"`）
...
- **清理格式空行和泛头 (对应 `del.py` 与 `remove_empty_lines.py`)**
  ```bash
  ./Evocg clean -i ./dataset/1
  ```

- **体积质控与过滤 (对应 `0-1.filterate_file_M1.py`)**
  计算指定目录中文件的平均大小，剔除并删除低于平均体积 90% 的蛋白文件，将剔除 ID 保存在 `remove_id.txt`。
  ```bash
  ./Evocg filter -i input/seq/prot_file/
  ```

- **多序列比对与格式转换 (对应 `clustal.pl` 与 `gcg2meg.pl`)**
  对指定文件夹中的 `.fa` 文件进行 Clustalw2 比对生成 `.gcg` 格式件，并将其转换为对应的 `.meg` (MEGA) 文件。
  ```bash
  ./Evocg clustal -d "output/CG_results/2.result"
  ```

### 3. 原生 EvoCG 2.0 内置算法模块

所有原版 EvoCG 2.0 的功能也已被完美内部整合。您可以以子命令的形式直接调用它们，且不用再理会任何路径冲突问题。

```bash
./Evocg cg            # 运行 EvoCG2.0 主算法逻辑
./Evocg bestpicker    # 运行 BestPicker 筛选逻辑
./Evocg batchprot     # 批量处理 ProtAcc 到 locTag
./Evocg combsingle    # 运行 combSingleGeneOrtho
./Evocg lenext        # 运行长度扩展逻辑 (lenExt)
./Evocg mutbest       # 运行 mutbest 处理
./Evocg protacc       # 处理单文件 ProtAcc 到 locTag
./Evocg simcov        # 运行相似度同源过滤 (simcovfilter)
```
